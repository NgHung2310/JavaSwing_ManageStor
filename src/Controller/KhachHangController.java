 ...
 * @author H
 */
public class KhachHangController implements Action {

    private KhachHangView view;
    private QuanLyKhoModel model;

    public KhachHangController(KhachHangView view) throws ClassNotFoundException, SQLException {
        this.view = view;
    ...
    public void actionPerformed(ActionEvent e) {
        try {
            ...
            switch (cmd) {
                case "Thêm":
                    if (Khachhang.isIdExist(view.getTextTfMaHang())) {
                        JOptionPane.showMessageDialog(view, "Mã này đã tồn tại!!");
                    } else {
                        Khachhang.InsertDB(view.getTextTfMaHang(), view.getTextTfTenHang(), view.getTextTfDonViTinh(), view.getTextTfSoLuong());
                ...
                case "Sửa":
                    if (view.getIndexTbl() < 0) {
                        ...
                        try {
                            Khachhang.UpdateDB(view.getTextSelectedId(), view.getTextTfMaHang(), view.getTextTfTenHang(), view.getTextTfDonViTinh(), view.getTextTfSoLuong());
                ...
                case "Xoá":
                    if (view.getIndexTbl() < 0) {
                        ...
                        if (JOptionPane.showConfirmDialog(view, "Bạn có chắc muốn xoá không?") == 0) {
                            Khachhang.DeleteDB(view.getTextSelectedId());
                    ...
                    break;
                case "Tìm":
                    Khachhang res = Khachhang.FindByID(view.getTextTfIdCanTim());
                    view.setJTable(res);
                    break;
                case "Làm mới":
                    view.loadJTable();
